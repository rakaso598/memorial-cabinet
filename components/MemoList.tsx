...
interface MemoListProps {
  ...
  onUpdateMemoTitle: (id: string, newTitle: string) => void;
  searchQuery: string;
...
const MemoList: React.FC<MemoListProps> = ({
  ...
  const handleTitleDoubleClick = (memo: Memo) => {
  ...
  const handleTitleBlur = (memoId: string) => {
  ...
  const handleTitleKeyDown = (
    ...
    if (e.key === "Enter") {
      (e.target as HTMLInputElement).blur(); // Enter 키 누르면 blur 이벤트 발생시켜 저장
    }
  };
  ...
  return (
    <aside className="w-80 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 p-4 overflow-y-auto shadow-inner">
      ...
      <div className="mb-4">
        <input
          type="text"
          placeholder="메모 검색..."
          className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-300 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500"
          value={searchQuery}
              ...
              className={`mb-2 p-3 rounded-lg cursor-pointer transition duration-200 ease-in-out transform hover:scale-[1.01]
                ${
                  selectedMemoId === memo.id
                    ? "bg-blue-100 border border-blue-400 shadow-md dark:bg-blue-800 dark:border-blue-600"
              ...
              <div className="flex justify-between items-center">
                {editingMemoId === memo.id ? (
                  <input
                    ...
                    autoFocus
                    className="flex-grow border rounded px-2 py-1 mr-2 text-lg font-medium text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-300 bg-white dark:bg-gray-700 placeholder-gray-400 dark:placeholder-gray-500"
